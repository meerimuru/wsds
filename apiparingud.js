...
async function listiraamatud() {
    try {
        const responseData = await getDataAsJson("https://wsdswebapp.azurewebsites.net/raamatud/");

        const resultElement = document.getElementById("raamatud_result");
        resultElement.innerHTML = "";

        for (var raamat of responseData.raamatud) {
            raamat = raamat.split('.')[0]; // Corrected variable name
            resultElement.innerHTML += `<a href="https://wsdswebapp.azurewebsites.net/raamatud/${raamat}" download="${raamat}.txt">${raamat}.txt</a> ` +
                `<a href="#" onclick="deleteObject('https://wsdswebapp.azurewebsites.net/raamatud/${raamat}')">[kustuta]</a>` +
                "<br />";
        }
    } catch (error) {
